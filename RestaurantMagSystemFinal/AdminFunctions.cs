...
namespace RestaurantMagSystemSecond
{
    public partial class AdminFunctions : Form
    {
        ...
        private void ShowPanel(Panel panelToShow = null, bool showDGV = false)
        {
            ...
            ShowTablesDGV.Visible = showDGV;

            // Show the requested panel if one is specified
            if (panelToShow != null)
            {
                panelToShow.Show();
            }
        ...
        {
            ShowPanel(ManageReservationsPanel);
        }
        ...
        {
            ShowPanel(DeleteAdPanel, showDGV: true);
        }

        private void DltUserBtn_Click(object sender, EventArgs e)
        {
            ShowPanel(DeleteUserPanel, showDGV: true);
        ...
        private void UserActivationPanel_Paint(object sender, PaintEventArgs e)
        {
        ...
        private void UpAdbtn_Click(object sender, EventArgs e)
        {
            if (UpAdnameTF.Text.Length <= 0 || UpAdpassTF.Text.Length <= 0 || UpAdWorkFTF.Text.Length <= 0)
            {
            ...
            else
            {
                Admin a = new Admin();
                LogInTableMembers mem = new LogInTableMembers();
              bool flag=  a.UpdateAdmin(UpAdnameTF.Text, UpAdpassTF.Text, UpAdWorkFTF.Text);
                if (flag)
                {
                    MessageBox.Show("Updation successful");
                    mem.UpdateAdminOnAdFunctions(UpAdnameTF.Text, UpAdpassTF.Text);
